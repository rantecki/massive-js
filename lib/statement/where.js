...
const autoGenerator = function (condition, conditions) {
  if (condition.isDocField) return docGenerator(condition, conditions);
  return tableGenerator(condition, conditions);
}
...
const generateConjunction = (criteria, offset, generator) => {
  return _.reduce(criteria, (conjunction, value, key) => {
    ...
    let isDocField = false;
    if (generator === 'autoGenerator') {
      if (key.startsWith('body.')) {
        key = key.slice('body.'.length);
        isDocField = true;
      }
    }
    ...
    if (generator === 'autoGenerator') condition.isDocField = isDocField;
