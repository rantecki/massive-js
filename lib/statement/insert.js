...
const Insert = function (source, record, options = {}) {
  ...
  if (this.document && this.junctions.length > 0) {
    // If this is marked as a document, we disable the default 'deep insert'
    // functionality.  Additionally, if we have a body column, then we map all
    // non-column fields into the body.
    if (this.source.columns.includes('body')) {
      // Ensure body is in the columns list.
      if (!this.columns.includes('body')) this.columns.push('body');
      // Map non-column fields into the body.
      this.records = this.records.map(
        r => _.defaults(_.omit(r, this.junctions), {
          body: _.defaults(_.pick(r, this.junctions), r.body || {})
        })
      );
    }
  ...
  if (!this.document && this.junctions.length) {
